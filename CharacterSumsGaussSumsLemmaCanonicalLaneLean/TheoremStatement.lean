...
structure GaussSumsAdmittedObject where
  field : Type
  additiveCharacter : Type
  multiplicativeCharacter : Type
  gaussSum : Type
  gaussSumFormula : Prop
  conclusion : gaussSumFormula

def GaussSumsWitnessClosed (O : GaussSumsAdmittedObject) : Prop :=
  O.gaussSumFormula
...
def mathlibProofObligation : MathlibProofObligation := {
  sourceKey := "CharacterSumsGaussSumsLemmaCanonicalLaneLean",
  theoremObject := "Character Sums Gauss Sums Lemma",
...
def theoremSpecificEndgamePilotClosed : Prop :=
  forall A : AdmissibleClass, ConstrainedGaussSumsClosure A

theorem theorem_specific_endgame_pilot_checked :
    theoremSpecificEndgamePilotClosed := by
  intro A
  exact constrained_gauss_sums_endgame A
